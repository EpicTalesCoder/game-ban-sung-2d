export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    for (let i = 0; i < 60; i++) {
      ...
      const y = Phaser.Math.Between(0, height - 80);
    ...
    for (let x = 0; x < width; x += 64) {
      this.ground.create(x, height - 16, 'ground').setOrigin(0, 0.5).refreshBody();
    }

    // Người chơi
    this.player = this.physics.add.sprite(120, height - 100, 'player');
    this.player.setCollideWorldBounds(true);
    this.player.body.setSize(28, 46).setOffset(6, 2);
  ...
  shoot(tx, ty) {
    ...
    const b = this.bullets.create(px, py, 'bullet');
    b.setRotation(ang);
  ...
  burst(x, y, color) {
    for (let i = 0; i < 8; i++) {
      const p = this.add.image(x, y, 'particle').setTint(color);
      const a = Phaser.Math.FloatBetween(0, Math.PI * 2);
      const s = Phaser.Math.Between(40, 160);
      this.tweens.add({
        targets: p, x: x + Math.cos(a) * s, y: y + Math.sin(a) * s,
